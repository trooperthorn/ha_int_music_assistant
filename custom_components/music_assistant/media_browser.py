...
def _bare_media_type(value: str) -> MASSMediaType | None:
    """Return the media type named by value, or None for anything unknown.

    The enum maps unknown names to MediaType.UNKNOWN instead of raising.
    """
    media_type = MASSMediaType(value)
    return None if media_type is MASSMediaType.UNKNOWN else media_type


def uri_media_type(uri: str) -> MASSMediaType | None:
    """Return the media type encoded in a Music Assistant item uri.

    Item uris have the form ``<provider>://<media_type>/<item_id>``; anything
    else (a library shortcut, a media source id, a plain url) yields None.
    """
    if "://" not in uri:
        return None
    _, _, remainder = uri.partition("://")
    media_type, separator, _ = remainder.partition("/")
    if not separator:
        return None
    return _bare_media_type(media_type)


def media_source_filter(item: BrowseMedia) -> bool:
    """Filter media sources."""
    return bool(item.media_content_type.startswith("audio/"))
...
async def async_browse_media(
    ...
    match uri_media_type(media_content_id) or _bare_media_type(media_content_id):
        case MASSMediaType.ARTIST:
            return await build_artist_items_listing(mass, media_content_id)
        case MASSMediaType.ALBUM:
            return await build_album_items_listing(mass, media_content_id)
        case MASSMediaType.PLAYLIST:
...
def _get_media_types_from_query(query: SearchMediaQuery) -> list[MASSMediaType]:
    ...
    allowed = (
        ...
        if uri_media_type(query.media_content_id or "") is MASSMediaType.ARTIST
...
async def async_search_media(
    ...
    try:
        ...
        if query.media_content_id:
            match uri_media_type(query.media_content_id):
                case MASSMediaType.ALBUM:
