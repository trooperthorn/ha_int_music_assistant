...
MASS_MEDIA_TYPE_TO_HA: Mapping[MediaType, HAMediaType] = {
    MediaType.PODCAST: HAMediaType.PODCAST,
    MediaType.PODCAST_EPISODE: HAMediaType.PODCAST,
    MediaType.PLAYLIST: HAMediaType.PLAYLIST,
}

MASS_ICON_TO_DEVICE_CLASS: Mapping[str, MediaPlayerDeviceClass] = {
    "tv": MediaPlayerDeviceClass.TV,
    "monitor": MediaPlayerDeviceClass.TV,
}
...
class MusicAssistantPlayer(MusicAssistantEntity, MediaPlayerEntity):
    ...
    def __init__(self, mass: MusicAssistantClient, player_id: str) -> None:
        ...
        self._attr_device_class = MASS_ICON_TO_DEVICE_CLASS.get(
            self.player.icon, MediaPlayerDeviceClass.SPEAKER
        )
    ...
    def _require_active_queue(self) -> PlayerQueue:
        """Return the active queue or raise a translated validation error."""
        if (queue := self.active_queue) is None:
            raise ServiceValidationError(
                translation_domain=DOMAIN,
                translation_key="no_active_queue",
                translation_placeholders={"player": self.player.name},
            )
        return queue
    ...
    async def async_set_volume_level(self, volume: float) -> None:
        ...
        await self.mass.players.player_command_volume_set(
            self.player_id, round(volume * 100)
        )
    ...
    async def async_set_shuffle(self, shuffle: bool) -> None:
        ...
        await self.mass.player_queues.queue_command_shuffle(
            self._require_active_queue().queue_id, shuffle
        )
    ...
    async def async_set_repeat(self, repeat: RepeatMode) -> None:
        ...
        await self.mass.player_queues.queue_command_repeat(
            self._require_active_queue().queue_id, MassRepeatMode(repeat)
        )
    ...
    async def async_clear_playlist(self) -> None:
        ...
        await self.mass.player_queues.queue_command_clear(
            self._require_active_queue().queue_id
        )
    ...
    async def async_play_media(
        ...
        **kwargs: Any,
        ...
        extra: Mapping[str, Any] = kwargs.get(ATTR_MEDIA_EXTRA) or {}
        if announce:
            await self._async_handle_play_announcement(
                media_id,
                use_pre_announce=extra.get(ATTR_USE_PRE_ANNOUNCE),
                pre_announce_url=extra.get(ATTR_PRE_ANNOUNCE_URL),
                announce_volume=extra.get(ATTR_ANNOUNCE_VOLUME),
            )
            return
        ...
        await self._async_handle_play_media(
            ...
            radio_mode=extra.get(ATTR_RADIO_MODE),
        )
    ...
    async def async_join_players(self, group_members: list[str]) -> None:
        ...
        player_ids = [
            get_player_id_for_entity(self.hass, child_entity_id)
    ...
    async def async_select_source(self, source: str) -> None:
        ...
        if source_id is None:
            raise ServiceValidationError(
                ...
                translation_placeholders={"source": source, "player": self.player.name},
            )
    ...
    async def async_select_sound_mode(self, sound_mode: str) -> None:
        ...
        if sound_mode_id is None:
            raise ServiceValidationError(
                translation_domain=DOMAIN,
                translation_key="sound_mode_not_found",
    ...
    async def _async_handle_transfer_queue(
        ...
        if not source_player:
            ...
            for queue in self.mass.player_queues:
                ...
                raise ServiceValidationError(
                    translation_domain=DOMAIN,
                    translation_key="no_playing_source",
                )
        else:
            source_queue_id = get_player_id_for_entity(self.hass, source_player)
    ...
    async def _async_handle_get_queue(self) -> ServiceResponse:
        ...
        active_queue = self._require_active_queue()
    ...
    def _update_media_attributes(
        ...
        self._attr_media_content_type = (
            MASS_MEDIA_TYPE_TO_HA.get(current_media.media_type, HAMediaType.MUSIC)
            if current_media is not None
            else HAMediaType.MUSIC
        )
